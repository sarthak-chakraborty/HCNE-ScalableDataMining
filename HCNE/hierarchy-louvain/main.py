...
for j in range(0,max(l)+1):
	...
	st1_comm=sys.argv[1][:-8]+str(j)+"_comm.txt"
	print "st1=",st1
	print "st1_comm=",st1_comm
	c=0
	...
	for line in f1:
		...
		if (t[0] in L[j] and t[1] not in L[j]) or (t[1] in L[j] and t[0] not in L[j]):
...
for j in range(0,max(l)+1):
	...
	f2_comm=open(st1[:-4]+"-Map_comm.txt",'w')	
	for line in f:
		t=line.split()
		if t[0] not in l1:
			l1.append(t[0])
	...
	for line in f_comm:
		t=line.split()
		if t[0] not in l1_comm:
			l1_comm.append(t[0])
		if t[1] not in l1_comm:
			l1_comm.append(t[1])
	...
	for k in range(0,len(l1)):
		...
		f2.write(str(k))
		f2.write("\n")
	for k in range(0,len(l1_comm)):
		f2_comm.write(str(l1_comm[k]))
		f2_comm.write(" ")
		f2_comm.write(str(k))
		f2_comm.write("\n")
	...
	for line in f_comm:
		t=line.split()
		f1_comm.write(str(l1_comm.index(t[0])))
		f1_comm.write(" ")
		f1_comm.write(str(l1_comm.index(t[1])))
		f1_comm.write("\n")
...
for j in range(0,max(l)+1):
	...
	os.system(st)
	st="./louvain"+" "+st1+".bin -l -1 -v >"+" "+st1+".tree"
	os.system(st)
