...
# 						D[node] = z
# 	else:
# 		st="python node2vec/src/main.py --input graph.txt --weighted --output node2vec/embed_enron1.txt"
# 		os.system(st)
# 		f=open("node2vec/embed_enron1.txt",'r')
...
# f1=open(sys.argv[1][:-1]+"-emb_comm_1.txt",'w')
...
while len(os.listdir(directory))>0:
	...
	for filename in os.listdir(directory):
		if(filename.endswith("-MapDet_comm.txt") and os.stat(directory+filename).st_size != 0):
			...
			f=open(directory+filename,'r')
			st="python node2vec/src/main.py --input "+directory+filename+" --output node2vec/embed_pfbcmu.txt"
			os.system(st)
			f=open("node2vec/embed_pfbcmu.txt",'r')
...
f1=open(sys.argv[1][:-1]+"-emb_comm_2.txt",'w')
f1.write(str(len(D.keys())))
f1.write(" ")
f1.write(str(128))
for k in D.keys():
