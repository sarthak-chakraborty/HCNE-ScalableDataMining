...
while len(os.listdir(directory))>0:
	...
	for filename in os.listdir(directory):
		if(filename.endswith("-MapDet_comm.txt") and os.stat(directory+filename).st_size != 0):
			...
			st="python node2vec/src/main.py --input "+directory+filename+" --output node2vec/embed_penron4.txt"
			os.system(st)
			f=open("node2vec/embed_penron4.txt",'r')
...
f1=open(sys.argv[1][:-1]+"-emb_comm_4.txt",'w')
