...
while len(os.listdir(directory))>0:
	...
	for filename in os.listdir(directory):
		if(filename.endswith("-MapDet_comm.txt") and os.stat(directory+filename).st_size != 0):
			...
			f=open(directory+filename,'r')
			st="python deepwalk/deepwalk/__main__.py --format edgelist --input "+directory+filename+" --output deepwalk/embed_fbcmu3.txt"
			os.system(st)
			f=open("deepwalk/embed_fbcmu3.txt",'r')
...
f1=open(sys.argv[1][:-1]+"-emb_comm_3_deepwalk.txt",'w')
